  ...
  return (
    <div className="auth-page">
      <div className="auth-card">
        <h1>看班智控台 Pro</h1>
        <p className="sub">SQLite 本地版 · 叫人 · 纪律 · 作业 · 点名 · 计时 · 宠物 · 值日 · 积分</p>
        ...
        <form className="stack" onSubmit={submit}>
          ...
          <button className="btn primary" disabled={busy} type="submit">
            {busy ? '处理中…' : mode === 'login' ? '进入班级' : mode === 'register' ? '提交注册' : '进入后台'}
          </button>
        </form>
        <p className="muted" style={{ marginTop: 14 }}>
          演示账号：班级 DEMO01 / 123456；管理员 admin / admin123。同一班级登录即可自动连上已在线的教室大屏。
        </p>
      </div>
    </div>
  );
